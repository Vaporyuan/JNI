...
public class ApkSignatureSchemeV2VerifierEx {
    ...
    public static class SignatureInfo {
        ...
        public ByteBuffer getSignatureBlock() {
    ...
    private static X509Certificate[] verifySigner(
        ...
        String jcaSignatureAlgorithm = signatureAlgorithmParams.getFirst();
        AlgorithmParameterSpec jcaSignatureAlgorithmParams = signatureAlgorithmParams.getSecond();
        boolean sigVerified;
        try {
            PublicKey publicKey =
    ...
    public static Pair<ByteBuffer, Long> getEocd(RandomAccessFile apk)
        ...
        Pair<ByteBuffer, Long> eocdAndOffsetInFile =
                ZipUtils.findZipEndOfCentralDirectoryRecords(apk);
    ...
    private static final int APK_SIGNATURE_SCHEME_V2_BLOCK_ID = 0x71536966;// 0x71536966; //0x7109871b
    //private static final int APK_SIGNATURE_SCHEME_V3_BLOCK_ID = 0x71536966;// 0x71536966; //0x7109871b
    private static final int APK_SIGNATURE_SCHEME_V3_BLOCK_ID = 0x7109871c;// 0x71536966; //0x7109871b
    // urovo weiyu add on 2020-12-09 [s]
    private static final int CITIC_APK_SIGNATURE_BLOCK_ID = 0x7109871c;
    private static final int CITIC_APK_SIGNATURE_SCHEME_V2_BLOCK_ID = 0x7109871a;
    // urovo weiyu add on 2020-12-09 [e]
    ...
    private static ByteBuffer findApkSignatureSchemeV2Block(ByteBuffer apkSigningBlock)
        ...
        while (pairs.hasRemaining()) {
            ...
            if (id == APK_SIGNATURE_SCHEME_V3_BLOCK_ID) {
    ...
    // urovo weiyu add on 2020-12-09 [s]
    /*get the whole Sign Data*/
    public static Pair<ByteBuffer, Long> findCiticApkSignatureSchemeV2Block(ByteBuffer apkSigningBlock)
            throws SignatureNotFoundException {
        checkByteOrderLittleEndian(apkSigningBlock);
        // FORMAT:
        // OFFSET       DATA TYPE  DESCRIPTION
        ...
        while (pairs.hasRemaining()) {
            ...
            int id = pairs.getInt();
            if (id == CITIC_APK_SIGNATURE_BLOCK_ID) {
                pairs.position(offset);
                ByteBuffer sgnBuf = getByteBuffer(pairs, len + 8 /*add the Block Len bytes*/);
                return Pair.create(sgnBuf, (long) offset + 8 /*add the Block Len bytes*/);
            }
            pairs.position(nextEntryPos);
        }
    ...
    /**
     * get the SGN Data from CITIC apk file
     *
     * @param apkFile
     * @return the InputStream of SGN Data
     */
    public static InputStream getCiticSgnData(String apkFile) throws IOException, SignatureNotFoundException {
        ...
        try {
            ...
            ByteBuffer apkSignatureSchemeV2Block = findCiticApkSignatureSchemeV2Block(apkSigningBlock).getFirst();
            apkSignatureSchemeV2Block.position(31 /*8+4+15+4 means: length + tag + magic + sign data len*/);
            byte[] sgnBytes = new byte[apkSignatureSchemeV2Block.capacity() - 31];
            apkSignatureSchemeV2Block.get(sgnBytes, 0, apkSignatureSchemeV2Block.capacity() - 31);

            ByteBuffer sgnData = ByteBuffer.wrap(sgnBytes);

            return new ByteArrayInputStream(sgnData.array());
        } finally {
            if (apk != null) {
                apk.close();
            }
    ...
    /**
     * get the SGN Data from apk file
     *
     * @param apkFile
     * @return the InputStream of SGN Data
     */
    public static InputStream getV3SgnData(String apkFile) throws IOException, SignatureNotFoundException {
        RandomAccessFile apk = null;
        try {
            apk = new RandomAccessFile(apkFile, "r");
            Pair<ByteBuffer, Long> eocdAndOffsetInFile = getEocd(apk);
            ...
            ByteBuffer apkSignatureSchemeV2Block = findApkSignatureSchemeV2Block(apkSigningBlock);
            //value data偏移
            int offset = 15 + 4; // V3中信 /*8+4+15+4 means: length + tag + magic + sign data len*/
            //int offset = 0; //8 + 4 + 15 + 4; // V3中信 /*8+4+15+4 means: length + tag + magic + sign data len*/
            apkSignatureSchemeV2Block.position(offset);
            byte[] sgnBytes = new byte[apkSignatureSchemeV2Block.capacity() - offset];
            apkSignatureSchemeV2Block.get(sgnBytes, 0, apkSignatureSchemeV2Block.capacity() - offset);
     ...
     * @return the InputStream of SGN Data
     */
    public static boolean hasV2Block(String apkFile) throws IOException, SignatureNotFoundException {
        RandomAccessFile apk = null;
        try {
            apk = new RandomAccessFile(apkFile, "r");
            Pair<ByteBuffer, Long> eocdAndOffsetInFile = getEocd(apk);
            ...
            return hasApkSignatureSchemeV2Block(apkSigningBlock);
    ...
    public static boolean hasApkSignatureSchemeV2Block(ByteBuffer apkSigningBlock)
            throws SignatureNotFoundException {
        checkByteOrderLittleEndian(apkSigningBlock);
        // FORMAT:
        // OFFSET       DATA TYPE  DESCRIPTION
        ...
        while (pairs.hasRemaining()) {
            ...
            int id = pairs.getInt();
            if (id == CITIC_APK_SIGNATURE_SCHEME_V2_BLOCK_ID) {
                return true;
            }
            pairs.position(nextEntryPos);
        }
